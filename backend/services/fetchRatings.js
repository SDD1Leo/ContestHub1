...
const fetchRatings = async (handles) => {
  ...
  if (handles.leetcode) {
    try {
      const lcRes = await axios.get(`http://localhost:5000/api/fetch/leetcode/${handles.leetcode}`);
  ...
  if (handles.codechef) {
    try {
      const ccRes = await axios.get(`http://localhost:5000/api/fetch/codechef/${handles.codechef}`);
