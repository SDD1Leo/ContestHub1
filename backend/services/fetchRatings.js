...
const fetchRatings = async (handles) => {
  ...
  if (handles.leetcode) {
    try {
      const lcRes = await axios.get(`https://contest-hub-server-sepia.vercel.app/api/fetch/leetcode/${handles.leetcode}`);
  ...
  if (handles.codechef) {
    try {
      const ccRes = await axios.get(`https://contest-hub-server-sepia.vercel.app/api/fetch/codechef/${handles.codechef}`);
